...
def add_d_leaves(t, v):
    ...
    def add_leaves(t,v,deep) :
        for i in t.branches :
            add_leaves(i,v,deep+1)
        num_of_leaves=0
        while num_of_leaves<deep :
...
def level_mutation_link(t, funcs):
	...
	if len(t.branches)==0 and remaining!=Link.empty :
...
def store_digits(n):
    ...
    if n//10==0 :
        ...
        return Link(n*10//div,store_digits(n%(div//10)))
...
def crispr_gene_insertion(lnk_of_genes, insert):
    ...
    def insertion(lnk_of_genes,insert,deep) :
        if lnk_of_genes==Link.empty :
            return 
        if isinstance(lnk_of_genes.first,Link) :
            link=lnk_of_genes.first
            codon=lnk_of_genes.first.first
            while codon!='AUG' and link.rest!=Link.empty:
                link=link.rest
                codon=link.first 
            if codon=='AUG' :
                for i in range(deep+1) :
                    new_link=Link(insert,link.rest)
                    link.rest=new_link
        else :
            if lnk_of_genes.first=='AUG' :
                for i in range(deep+1) :
                    lnk_of_genes.first+=' '
                    lnk_of_genes.first+=insert
        insertion(lnk_of_genes.rest,insert,deep+1)
    insertion(lnk_of_genes,insert,0)
...
def transcribe(dna):
    ...
    return [dict[dna[i]]+dict[dna[i+1]]+dict[dna[i+2]] for i in range(0,len(dna)-2,3)] 
