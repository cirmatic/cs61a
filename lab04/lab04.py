...
def merge(s, t):
    ...
    if len(s)>0 and len(t)>0 :
        if s[0]>t[0] :
            result=list(map(int,str(t[0])))+merge(s,t[1:])
        else :
            result=list(map(int,str(s[0])))+merge(s[1:],t)
    else :
        if len(s)>0 :
            result=s
        else :
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    new_t.append(label((t)))
    for i in branches(t) :
        if is_leaf(i):
            if i[0]=="loki" :
                k=copy_tree(i)
                k[0]=lokis_replacement
                new_t.append(k)
            else :
                new_t.append(i)
        else :
            new_t.append(replace_loki_at_leaf((i),lokis_replacement))
    return new_t
