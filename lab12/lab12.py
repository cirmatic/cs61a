...
def subseqs(s):
    ...
    if len(s)==0 :
        ...
        with_first_ele=insert_into_all(s[0],subseqs(s[1:]))
        with_first_ele.extend(subseqs(s[1:]))
        return with_first_ele 

def non_dec_insert_into_all(item, nested_list):
    """Return a new list consisting of all the lists in nested_list,
    but with item added to the front of each. You can assume that
     nested_list is a list of lists.

    >>> nl = [[], [1, 2], [3]]
    >>> non_dec_insert_into_all(2, nl)
    [[2], [2, 3]]
    """
...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s):
        if not s:
            ...
            a = subseq_helper(s[1:])
            b = s[0]
            return non_dec_insert_into_all(b, a) + a
...
if __name__ == "__main__" :
    ...
    doctest.run_docstring_examples(non_dec_insert_into_all,globals())
